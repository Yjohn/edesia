exports.seed = async (knex, Promise) => {
  ...
  await knex('stores').del().then(function () {
    // Inserts seed entries
  ...
  await knex('deliveries').del().then(function () {
    // Inserts seed entries
    return knex('deliveries').insert([
      {
        address: 'Kinning Park',
        store_name: 'KPC',
        deadline: new Date('2018-10-26T03:30:00'),
        status: 'Available',
        driver_id: 1,
        latitude: '55.849055',
        longitude: '-4.282586'
  ...
  await knex('status').del().then(function () {
    // Inserts seed entries
